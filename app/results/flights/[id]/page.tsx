    ...
    <main className="mx-auto max-w-3xl px-4 py-10">
      ...
      <div className="rounded-2xl border border-outline-variant bg-surface-container-low overflow-hidden">
        ...
        <div className="flex items-center gap-4 p-6 border-b border-outline-variant bg-surface-container">
          ...
          <div>
            <h1 className="text-xl font-bold text-on-surface">{f.airline}</h1>
            <p className="text-sm text-on-surface-variant">{f.airlineCode} · {f.cabin}</p>
          </div>
        ...
        <div className="p-6 flex flex-col gap-6">
          {/* Outbound */}
          <section>
            <p className="text-xs font-semibold text-on-surface-variant uppercase tracking-wide mb-3">Outbound flight</p>
            <div className="flex items-center gap-4">
              <div className="text-center w-20">
                <p className="text-2xl font-bold text-on-surface">{formatTime(f.departureTime) || "—"}</p>
                <p className="text-sm text-on-surface-variant">{f.departureAirport}</p>
                <p className="text-xs text-on-surface-variant">{formatDate(f.departureTime)}</p>
              </div>
              <div className="flex-1 flex flex-col items-center gap-1">
                ...
                <p className="text-xs text-on-surface-variant">{f.stops === 0 ? "Non-stop" : f.stops === 1 ? "1 stop" : `${f.stops} stops`}</p>
              </div>
              <div className="text-center w-20">
                <p className="text-2xl font-bold text-on-surface">{formatTime(f.arrivalTime) || "—"}</p>
                <p className="text-sm text-on-surface-variant">{f.arrivalAirport}</p>
                <p className="text-xs text-on-surface-variant">{formatDate(f.arrivalTime)}</p>
              </div>
            </div>
          </section>
          ...
          {f.returnFlight && (
            <section>
              <p className="text-xs font-semibold text-on-surface-variant uppercase tracking-wide mb-3">Return flight</p>
              <div className="flex items-center gap-4">
                <div className="text-center w-20">
                  <p className="text-2xl font-bold text-on-surface">{formatTime(f.returnFlight.departureTime) || "—"}</p>
                  <p className="text-sm text-on-surface-variant">{f.arrivalAirport}</p>
                </div>
                <div className="flex-1 flex flex-col items-center gap-1">
                  <p className="text-xs text-on-surface-variant">{formatDuration(f.returnFlight.durationMinutes)}</p>
                  <div className="w-full h-px bg-outline-variant" />
                  <p className="text-xs text-on-surface-variant">{f.returnFlight.stops === 0 ? "Non-stop" : `${f.returnFlight.stops} stop${f.returnFlight.stops > 1 ? "s" : ""}`}</p>
                </div>
                <div className="text-center w-20">
                  <p className="text-2xl font-bold text-on-surface">{formatTime(f.returnFlight.arrivalTime) || "—"}</p>
                  <p className="text-sm text-on-surface-variant">{f.departureAirport}</p>
                </div>
              </div>
            </section>
          ...
          <section className="grid grid-cols-2 gap-3 text-sm">
            <div className="bg-surface-container rounded-xl p-3">
              <p className="text-xs text-on-surface-variant mb-1">Cabin</p>
              <p className="font-medium text-on-surface">{f.cabin}</p>
            </div>
            <div className="bg-surface-container rounded-xl p-3">
              <p className="text-xs text-on-surface-variant mb-1">Baggage</p>
              <p className="font-medium text-on-surface">{f.baggage}</p>
            </div>
          </section>
...
function NotFound({ tripId }: { tripId: string }) {
  return (
    <main className="mx-auto max-w-3xl px-4 py-10">
      ...
      <p className="text-on-surface-variant">Flight not found.</p>
    </main>
  );
}
