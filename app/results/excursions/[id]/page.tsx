      ...
      <div className="rounded-lg border border-hairline bg-canvas overflow-hidden">
        <div className="p-6 border-b border-hairline">
          <div className="flex items-start justify-between gap-3 flex-wrap">
            <div>
              <h1 className="text-xl font-bold text-ink">{e.title}</h1>
              <p className="text-sm text-steel mt-0.5 capitalize">{e.category} · {e.durationHours}h · {e.location}</p>
            </div>
        ...
        <div className="p-6 flex flex-col gap-6">
          <p className="text-sm text-steel leading-relaxed">{e.description}</p>

          <section className="grid grid-cols-2 gap-3 text-sm">
            <div className="bg-surface rounded-xl p-3">
              <p className="text-xs text-steel mb-1">Date</p>
              <p className="font-medium text-ink">{formatDate(e.date)}</p>
            </div>
            <div className="bg-surface rounded-xl p-3">
              <p className="text-xs text-steel mb-1">Duration</p>
              <p className="font-medium text-ink">{e.durationHours} hours</p>
            </div>
            {e.groupSize && (
              <div className="bg-surface rounded-xl p-3 col-span-2">
                <p className="text-xs text-steel mb-1">Group size</p>
                <p className="font-medium text-ink">{e.groupSize}</p>
              </div>
            )}
          </section>
          ...
          {e.includes.length > 0 && (
            <section>
              <p className="text-xs font-semibold text-steel uppercase tracking-wide mb-2">What&apos;s included</p>
              <div className="flex flex-wrap gap-2">
                {e.includes.map((item) => (
                  <span key={item} className="rounded-full bg-surface px-3 py-1 text-xs text-steel">
                    {item}
                  </span>
                ))}
              </div>
...
function NotFound({ tripId }: { tripId: string }) {
  return (
    <main className="mx-auto max-w-[840px] px-4 sm:px-6 py-6 sm:py-10">
      ...
      <p className="text-steel">Excursion not found.</p>
    </main>
  );
}
