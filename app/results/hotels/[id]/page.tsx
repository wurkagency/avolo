...
export default function HotelDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-10">
      ...
      {galleryPhotos[0] && (
        <div className="rounded-lg overflow-hidden h-64 mb-6 bg-surface">
      ...
      <div className="rounded-lg border border-hairline bg-canvas overflow-hidden">
        ...
        <div className="p-6 border-b border-hairline">
          <div className="flex items-start justify-between gap-3 flex-wrap">
            <div>
              <h1 className="text-xl font-bold text-ink">{h.name}</h1>
              ...
              {h.address && <p className="text-xs text-steel mt-1">{h.address}</p>}
            </div>
          ...
          {h.rating !== null && (
            <div className="flex items-center gap-2 mt-3">
              ...
              {h.reviewCount !== null && <span className="text-xs text-steel">{h.reviewCount.toLocaleString()} reviews</span>}
            </div>
          )}
        </div>
        ...
        <div className="p-6 flex flex-col gap-6">
          ...
          <section className="grid grid-cols-2 gap-3 text-sm">
            <div className="bg-surface rounded-xl p-3">
              <p className="text-xs text-steel mb-1">Check-in</p>
              <p className="font-medium text-ink">{formatDate(h.checkIn)}</p>
            </div>
            <div className="bg-surface rounded-xl p-3">
              <p className="text-xs text-steel mb-1">Check-out</p>
              <p className="font-medium text-ink">{formatDate(h.checkOut)}</p>
            </div>
            <div className="bg-surface rounded-xl p-3">
              <p className="text-xs text-steel mb-1">Room type</p>
              <p className="font-medium text-ink">{h.roomType}</p>
            </div>
            <div className="bg-surface rounded-xl p-3">
              <p className="text-xs text-steel mb-1">Cancellation</p>
              <p className="font-medium text-ink capitalize">{h.cancellationPolicy}</p>
            </div>
            {h.breakfast && (
              <div className="bg-surface rounded-xl p-3 col-span-2">
                <p className="text-xs text-steel mb-1">Breakfast</p>
                <p className="font-medium text-ink">Included</p>
              </div>
            )}
          </section>
          ...
          <div className="flex items-center justify-between border-t border-hairline pt-4 flex-wrap gap-3">
            <div>
              ...
              <p className="text-xs text-steel">total for {h.nights} night{h.nights !== 1 ? "s" : ""}</p>
...
function NotFound({ tripId }: { tripId: string }) {
  return (
    <main className="mx-auto max-w-3xl px-4 py-10">
      ...
      <p className="text-steel">Hotel not found.</p>
    </main>
  );
}
