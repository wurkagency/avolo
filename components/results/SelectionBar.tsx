...
export function SelectionBar() {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-hairline bg-surface/95 backdrop-blur-sm shadow-lg">
      <div className="mx-auto max-w-5xl px-4 py-3 flex items-center gap-4 flex-wrap">
        <div className="flex items-center gap-3 flex-1 min-w-0 flex-wrap">
          {selected.map(([type, item]) => item && (
            <span key={type} className="flex items-center gap-1.5 text-sm text-ink">
              <span
              ...
              <span className="text-steel truncate max-w-[120px]">{item.summary}</span>
            </span>
          ))}
        </div>
